...
from math import dist, floor, log
    ...
    for number in numbers:
        ...
        for i in range(len(boards)):
            ...
            if checkForWinningBoard(board):
                indexlastBoardToWin.append(i)
        if len(indexlastBoardToWin) > 0:
            lastBoard = {}
            for x in sorted(indexlastBoardToWin, reverse=True):
                lastBoard = boards.pop(x)
            indexlastBoardToWin = []
            if len(boards) == 0:
                return {"board": lastBoard, "number": number}
